...
// Hook special pentru featured article — se declanșează devreme (înainte să intre în viewport)
function useRevealEarly() {
  const [visible, setVisible] = useState(false);
  const ref = useRef<HTMLDivElement>(null);
  useEffect(() => {
    const obs = new IntersectionObserver(
...
export default function ArticlesPreview() {
  ...
  const { ref: featuredRef, visible: featuredVisible } = useRevealEarly();
  const featured = articles[0];
  const rest = articles.slice(1);

  // Detectăm mobil pentru animație diferită (fără clip-path care creează spațiu alb)
  ...
  return (
    <section id="articole" className="pt-24 lg:pt-32 pb-12 lg:pb-14 bg-brand-cream overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        ...
        {/* Featured article — clip-path pe desktop, opacity+transform pe mobil */}
        <div
          ref={featuredRef}
          ...
          style={
            isMobile
              ? {
                  ...
                  transform: featuredVisible ? 'translateY(0)' : 'translateY(24px)',
                  transition: 'opacity 0.7s ease 80ms, transform 0.7s ease 80ms',
                }
              : {
                  clipPath: featuredVisible
